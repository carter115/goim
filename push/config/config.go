...
func InitConfig() (err error) {
	...
	Redis.Ttl = Redis.Ttl * time.Hour

	// load mongo Section
	if err = cfg.Section("mongo").MapTo(&Mongo); err != nil {
		return
	}
	Mongo.ConnTimeout = Mongo.ConnTimeout * time.Millisecond
...
func String() string {
	return fmt.Sprintf("App:%+v, Rpc:%+v Logger:%+v, Kafka:%+v", App, Rpc, Logger, Kafka)
}
